# Controle-de-investimentos-com-Excel

Projeto desenvolvido em Excel para simular a evolução de investimentos recorrentes em Fundos de Investimento Imobiliário (FIIs), permitindo visualizar o patrimônio acumulado, uma estimativa de dividendos mensais e uma sugestão de distribuição do aporte mensal entre diferentes tipos de FIIs.


<img width="1379" height="388" alt="finanças fácil 2" src="https://github.com/user-attachments/assets/c92f87e6-1a7f-41fd-9107-e148b59e5944" />



## 🎯 Objetivo
Este projeto tem como objetivo oferecer uma ferramenta prática em Excel para simulação de investimentos em **fundos imobiliários (FIIs)**. A planilha permite calcular:
- Valor total investido ao longo do tempo  
- Patrimônio acumulado  
- Dividendos mensais projetados  
- Cenários de crescimento em diferentes horizontes (2, 5, 10, 20, 30 anos)  

### ⚙️ Configurações
Os dados preenchidos são apenas exemplos, mas você pode personalizar:
- **Salário mensal**  
- **Percentual sugerido de investimento (30%)**  
- **Valor de investimento mensal**  
- **Horizonte de tempo (anos)**  
- **Taxa de rendimento mensal**  

Exemplo da planilha:  
- Salário: R$ 2.000  
- Investimento mensal: R$ 200  
- Prazo: 5 anos  
- Taxa de rendimento: 1,07% ao mês  

### 📊 Perfis de Investidor
A planilha sugere alocação em diferentes tipos de FIIs conforme o perfil:

| **Perfil** | **Papel** | **Tijolo** | **Híbridos** | **FOFs** | **Desenvolvimento** | **Hotelarias** |
|----------------|---------|---------|---------|---------|---------|---------|
| Conservador    | 30%     | 50%     | 10%     | 10%     | 0%      | 0%      |
| Moderado       | 32%     | 35%     | 8%      | 5%      | 10%     | 10%     |
| Agressivo      | 50%     | 10%     | 5%      | 5%      | 20%     | 10%     |

### 📈 Cenários de Dividendos
A planilha projeta dividendos mensais e acumulados em diferentes horizontes:
- 2 anos → R$ 5  
- 5 anos → R$ 10  
- 10 anos → R$ 20  
- 20 anos → R$ 30  

### 🛠️ Recursos Utilizados
- Fórmulas financeiras do Excel (juros compostos, projeções de dividendos)  
- Estrutura de tabelas dinâmicas para perfis de investidor  
- Automação de cálculos para patrimônio acumulado  

### 🚀 Como Usar
1. Abra o arquivo **Simulador_Investimentos_Fundos_Imobiliarios_Excel.xlsx**  
2. Preencha os campos de configuração (salário, investimento mensal, prazo, taxa de rendimento)  
3. Consulte os cenários de dividendos e patrimônio acumulado  
4. Compare os perfis de investidor para definir sua estratégia  

---
